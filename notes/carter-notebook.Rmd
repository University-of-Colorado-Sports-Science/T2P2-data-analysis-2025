...
lb_injury_B <- incid_b %>%
  distinct(anon_id) %>%
  pull(anon_id)
acl_injury_B <- incid_b %>%
  filter(OSICS14.Code == 'KLA') %>%
  distinct(anon_id) %>%
  pull(anon_id)
hsi_injury_B <- incid_b %>%
  filter(OSICS14.Code == 'TM1') %>%
  distinct(anon_id) %>%
  pull(anon_id)
ankle_injury_B <- incid_b %>%
...
b['Ankle'] <- ifelse(b$anon_id %in% ankle_injury_B, 'Yes', 'No')
```
...
lb_injury_S <- incid_s %>%
  distinct(anon_id) %>%
  pull(anon_id)
acl_injury_S <- incid_s %>%
  filter(OSICS14.Code == 'KLA') %>%
  distinct(anon_id) %>%
  pull(anon_id)
hsi_injury_S <- incid_s %>%
  filter(OSICS14.Code == 'TM1') %>%
  distinct(anon_id) %>%
  pull(anon_id)
ankle_injury_S <- incid_s %>%
  filter(Body.Part == 'Ankle') %>%
  distinct(anon_id) %>%
  pull(anon_id)
```
...
s['Ankle'] <- ifelse(s$anon_id  %in% ankle_injury_S, 'Yes', 'No')
```
...
lb_injury_L <- incid_l %>%
  distinct(anon_id) %>%
  pull(anon_id)
acl_injury_L <- incid_l %>%
  filter(OSICS14.Code == 'KLA') %>%
  distinct(anon_id) %>%
  pull(anon_id)
hsi_injury_L <- incid_l %>%
  filter(OSICS14.Code == 'TM1') %>%
  distinct(anon_id) %>%
  pull(anon_id)
ankle_injury_L <- incid_l %>%
  filter(Body.Part == 'Ankle') %>%
  distinct(anon_id) %>%
  pull(anon_id)
```
...
l['Ankle'] <- ifelse(l$anon_id %in% ankle_injury_L, 'Yes', 'No')
```
...
lb_injury_V <- incid_v %>%
  distinct(anon_id) %>%
  pull(anon_id)
acl_injury_V <- incid_v %>%
  filter(OSICS14.Code == 'KLA') %>%
  distinct(anon_id) %>%
  pull(anon_id)
hsi_injury_V <- incid_v %>%
  filter(OSICS14.Code == 'TM1') %>%
  distinct(anon_id) %>%
  pull(anon_id)
ankle_injury_V <- incid_v %>%
  filter(Body.Part == 'Ankle') %>%
  distinct(anon_id) %>%
  pull(anon_id)
```
...
v['Ankle'] <- ifelse(v$anon_id %in% ankle_injury_V, 'Yes', 'No')
```
...
nordbord_hj <- rbind(HJ_B, HJ_S, HJ_L, HJ_V) %>%
  mutate(Sport.2 = ifelse(Sport == 'Basketball' | Sport == 'Volleyball', 'Court', 'Field'),
         Best.Avg.Force.Asym = abs(Best.Average.Force..Asym...N.),
         Best.Peak.Force.Asym = abs(Best.Peak.Force..Asym...N.),
         Mean.Landing.Force.Asym = abs(Mean.Landing.RFD..Asym...N.s.))
...
```{r}
ggplot(data = nordbord_hj, aes(x = Best.Time.to.Peak.Force..ms., fill = Ankle)) + 
  geom_density(alpha = 0.5) + 
  scale_fill_manual(values = c('#CFB87C', 'black')) + 
...
```{r}
ggplot(data = nordbord_cmj, aes(x = Peak.Power...BM..W.kg., fill = Ankle)) + 
  geom_density(alpha = 0.5) + 
  scale_fill_manual(values = c('#CFB87C', 'black')) + 
  labs(title = 'Distribution of Hop Jump RSI vs Ankle Injury (All Time)',
       y = 'Density',
...
```{r}
ggplot(data = hip_all_injury, aes(Hip.Abduction.MEAN.Imbalance, fill = Was.Injured)) + 
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = 1.4, color = 'red', linetype = 'dashed') + 
  scale_fill_manual(values = c('#CFB87C', '#000000')) +
...
cmj_all_injury <- rbind(merge_cmj_B, merge_cmj_S, merge_cmj_L, merge_cmj_V) %>%
  mutate(Sport.2 = ifelse(Sport == 'Basketball' | Sport == 'Volleyball', 'Court', 'Field'),
         Was.Injured = ifelse(Was.Injured == 'Yes', 'Yes', 'No'),
         Peak.Landing.Asym = abs(Peak.Landing.Force..Asym...N.))
```
...
```{r}
ggplot(data = cmj_all_injury, aes(Landing.Net.Peak.Force...BM..N.kg., fill = Was.Injured)) + 
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c('#CFB87C', '#000000')) +
...
train <- cmj_all_injury %>%
  mutate(LB = as.factor(ifelse(Was.Injured == 'Yes', 1, 0)),
         AKL = as.factor(ifelse(Area == 'Ankle', 1, 0)),
         KNEE = as.factor(ifelse(Area == 'Knee', 1, 0))) %>%
  dplyr::select(-c(anon_id, Date, Injury, Area, Was.Injured, Sport, Sport.2, AKL, KNEE))

model_null <- glm(LB ~ 1, data = train, family = 'binomial')
model_step <- glm(LB ~ ., data = train, family = 'binomial')
...
hj_all_injury <- rbind(merge_hj_B, merge_hj_S, merge_hj_L, merge_hj_V) %>%
  mutate(Sport.2 = ifelse(Sport == 'Basketball' | Sport == 'Volleyball', 'Court', 'Field'),
         ...
         Was.Injured. = as.factor(ifelse(Was.Injured == 'Yes', 1, 0)),
         HJ.RSI.FLAG = as.factor(ifelse(HJ.RSI < 1.05, 1, 0)),
         Flight.Time.FLAG = as.factor(ifelse(Mean.Flight.Time..ms. < 0.4245, 1, 0)),
         Contact.Time.FLAG = as.factor(ifelse(Mean.Contact.Time..ms. > 0.2085, 1, 0)),
         Jump.Height.FLAG = as.factor(ifelse(Max.Jump.Height..cm. < 21.5725, 1, 0)))
...
```{r}
ggplot(data = hj_all_injury, aes(Mean.Flight.Time..ms., fill = Was.Injured)) + 
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c('#CFB87C', '#000000')) +
  geom_vline(xintercept = 0.4245, color = 'red', linetype = 'dashed') +
  labs(title = 'Mean Flight Time vs Lower Body Injury',
       x = 'Mean Flight Time (s)',
       y = 'Density',
       fill = 'Lower Body Injury',
       caption = 'CU Womens Athletes') +
  scale_x_continuous(breaks = seq(0, 50, 0.04)) + 
  theme_bw()
```
...
```{r}
model <- glm(Was.Injured. ~ HJ.RSI.FLAG + Flight.Time.FLAG + Contact.Time.FLAG + Jump.Height.FLAG, data = hj_all_injury, family = 'binomial')
...
hj %>%
  ...
  summarize(`RSI` = mean(HJ.RSI),
            ...
            `Other RSI` = mean(Mean.RSI..Flight.Contact.Time.))
```

```{r}
#creates a density plot for maximum force across sports
ggplot(hj, aes(x = Mean.RSI..Flight.Contact.Time., color = Sport, fill = Sport)) + 
  geom_density(alpha = 0.4) + 
  scale_fill_manual(values = c('#CFB87C', '#60D272', '#FF736F', '#565A5C')) +
  scale_color_manual(values = c('#CFB87C', '#60D272', '#FF736F', '#565A5C')) +
  labs(title = 'Distribution of Mean RSI (Hop Jump) Across Sports',
       x = 'Mean RSI (m/s)',
       y = 'Density') +
  scale_x_continuous(breaks = seq(from = 0, to = 7, by = 0.2)) + 
  theme_bw()
...
```{r}
#creates a density plot for maximum force across sports
ggplot(hj, aes(x = Mean.RSI..Flight.Contact.Time., color = Sport.2, fill = Sport.2)) + 
  geom_density(alpha = 0.5) + 
  scale_fill_manual(values = c('#CFB87C', '#000000')) +
  scale_color_manual(values = c('#CFB87C', '#000000')) +
  labs(title = 'Distribution of Mean RSI (Hop Jump) Across Sports',
...
```{r}
img <- image_read("C:/Users/cazb4905/Documents/T2P2-data-analysis-2025/hjrsi_wlx.png")
img_grob <- rasterGrob(img, interpolate = TRUE)
img_plot <- ggdraw() + draw_grob(img_grob, x = 0, y = 0, width = 1, height = 1)

p <- ggplot(HJ_L, aes(x = Mean.RSI..Flight.Contact.Time.)) + 
  geom_density(fill = '#CFB87C') +
  labs(title = 'Distribution of Mean RSI (Flight Contact Time)',
       x = 'Mean RSI (m/s)',
       y = 'Density',
       caption = 'CU Womens Lacrosse') +
  geom_vline(xintercept = median(HJ_L$Mean.RSI..Flight.Contact.Time.), linetype = 'solid') +
  geom_vline(xintercept = c(quantile(HJ_L$Mean.RSI..Flight.Contact.Time., probs = c(0.01, 0.25, 0.75, 0.99))), linetype = 'dashed') + 
  scale_x_continuous(breaks = seq(from = 0, to = 100, by = 0.2)) + 
  theme_bw()
...
```{r}
quantile(HJ_L$Mean.RSI..Flight.Contact.Time., probs = c(0.25, 0.5, 0.75))
```

```{r}
t.test(Mean.RSI..Flight.Contact.Time. ~ Sport, data = hj %>% filter(Sport %in% c('Soccer', 'Volleyball')))
```
