...
ggplot(cmj, aes(x = Landing.Net.Peak.Force...BM..N.kg., color = Sport, fill = Sport)) + 
...
```{r}
ggplot(HJ_L, aes(x = Mean.Contact.Time..ms.)) + 
...
intercepts <- data.frame(
  Sport = c('Soccer', 'Lacrosse'),
  Median = c(median(performance_S_nordic$Maximum.Nordic.Bilateral.Mean), median(performance_L_nordic$Maximum.Nordic.Bilateral.Mean)),
  Percentiles = c(quantile(performance_S_nordic$Maximum.Nordic.Bilateral.Mean, 0.01), quantile(performance_L_nordic$Maximum.Nordic.Bilateral.Mean, 0.01), quantile(performance_S_nordic$Maximum.Nordic.Bilateral.Mean, 0.99), quantile(performance_L_nordic$Maximum.Nordic.Bilateral.Mean, 0.99), quantile(performance_S_nordic$Maximum.Nordic.Bilateral.Mean, 0.25), quantile(performance_L_nordic$Maximum.Nordic.Bilateral.Mean, 0.25), quantile(performance_S_nordic$Maximum.Nordic.Bilateral.Mean, 0.75), quantile(performance_L_nordic$Maximum.Nordic.Bilateral.Mean, 0.75))
)

#creates a density plot for maximum bilateral force across sport type
ggplot(field_all, aes(x = Maximum.Nordic.Bilateral.Mean)) + 
  geom_density(fill = '#CFB87C') + 
  labs(title = 'Distribution of Maximum Nordic Bilateral Force',
       ...
       caption = 'CU Field Sports') +
  facet_wrap(~Sport) + 
  geom_vline(data = intercepts, aes(xintercept = Median), linetype = 'solid') +
  geom_vline(data = intercepts, aes(xintercept = Percentiles), linetype = 'dashed') + 
  theme_bw()
```
