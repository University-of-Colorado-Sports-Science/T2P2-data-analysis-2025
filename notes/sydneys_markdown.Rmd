...
```{r}
#BBALL CMJ RSI by LB injury
ggplot(B_incid_CMJtest,aes(as.factor(x=LB.Injury),y=Max.CMJ.RSI.Modified..m.s., color=LB.Injury))+ 
...
all_CMJ <- rbind(CMJ_bb, CMJ_soc, CMJ_lax, CMJ_vb) #%>%
 # mutate(Thresh = ifelse(Variable >= #, 'Yes', 'No'))
all_CMJ_incident <- merge(all_incident, all_CMJ,by=c('anon_id'), na.rm=TRUE)
```

```{r}
#create a dataset containing all HJ test data across all sports
all_hopjump <- rbind(HJ_B, HJ_S, HJ_L, HJ_V)
  #mutate(Thresh = ifelse(Relative.Maximum.Nordic.Bilateral.Mean >= #, 'Yes', 'No'))
all_hopjump_incident <- merge(all_incident, all_hopjump,by=c('anon_id'), na.rm=TRUE)
```

```{r}
#create a dataset containing all IMTP test data across all sports
all_IMTP <- rbind(IMTP_B, IMTP_S, IMTP_L, IMTP_V)
all_IMTP_incident <- merge(all_incident, all_IMTP,by=c('anon_id'), na.rm=TRUE)
```

```{r}
#create a dataset containing all SLJ test data across all sports
all_SLJ <- rbind(SLJ_s, SLJ_l) 
all_SLJ_incident <- merge(all_incident, all_SLJ,by=c('anon_id'), na.rm=TRUE)
```
...
1. CMJ + Incident 
```{r}
#choose variables thresholds
library(cutpointr)
cp <- cutpoint(all_CMJ_incident,Eccentric.Mean.Braking.Force..N, Eccentric.Mean.Braking.Force.Thresh, method="maximize_metric", metric=youden)
plot(cp)
```

```{r}
library(ROCR)
roc_curve <- roc(all_CMJ_incident, x=Eccentric.Mean.Braking.Force..N, pos_class="Yes", neg_class="No", direction=">")
plot_roc(roc_curve)
auc(roc_curve)
```
...
all_CMJ_incident <- all_CMJ_incident %>%
 mutate(Thresh = ifelse(Relative.Concentric.Power.Bilateral >= 0, 'Yes', 'No'))

#creates a table with lower body injury on left and whether the observation is above/below threshold
table(all_CMJ_incident$LB, all_CMJ_incident$Thresh)
...
ggplot(all_CMJ_incident, aes(x = Avg.Add.Max, fill = Sport)) +
...
anova_result <- aov(Maximum.Force ~ Sport, data = all_CMJ_incident)
summary(anova_result)

#alos do mx RSI
#concentric maximum RFD, eccentric mean braking force 
```
