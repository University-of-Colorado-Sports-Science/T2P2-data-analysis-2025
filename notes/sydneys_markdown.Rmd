...
```
In womens' volleyball 60 lower body injuries occurred, 10 of them being ACL injuries, 4 of them being HSI injuries. 
...
```{r}
#lacrosse incidents + tests

```

```{r}
# volleyball incidents + tests
...
```{r}
#ALL SOCCER DATA //NEED TO ADD SLJ
...
soc_merge_full <- merge(S_merge,IMTP_S, by=c('anon_id','Date'), all=TRUE)   #213 by 110
```

```{r}
#ALL LACROSSE DATA    //NEED TO ADD SLJ
...
#ALL VOLLEYBALL DATA
soccer_merge <- merge(performance_S_nordic,performance_S_hip, by=c('anon_id','Date'), all=TRUE)
soc_merge <- merge(basketball_merge,incident_s, by=c('anon_id','Date'), all=TRUE)
s_merge <- merge(basket_merge,HJ_S, by=c('anon_id','Date'), all=TRUE)
S_merge <- merge(bball_merge,CMJ_soc, by=c('anon_id','Date'), all=TRUE)
soc_merge_full <- merge(bball_merge,IMTP_S, by=c('anon_id','Date'), all=TRUE)
```
