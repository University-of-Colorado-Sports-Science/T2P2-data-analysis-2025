...
IMTP_S <- Forcedecks_S %>%
  ...
  mutate(Date = as.Date(Date, format = '%m/%d/%Y')) %>%
  na.omit() %>%
  mutate(Sport = 'Soccer')   #68 obs & 30 vars
```
...
IMTP_L <- Forcedecks_L %>%
  ...
  mutate(Date = as.Date(Date, format = '%m/%d/%Y')) %>%
  na.omit() %>%
  mutate(Sport = 'Lacrosse')  #33 obs & 30 vars
```
...
IMTP_V <- Forcedecks_V %>%
  ...
  mutate(Date = as.Date(Date, format = '%m/%d/%Y')) %>%
  na.omit() %>%
  mutate(Sport = 'Volleyball')  #249 obs & 30 vars
```
...
summary(cp)
```
For Peak Vertical Force the optimal cutpoint is 30.813 with a AUC of 0.5116, sensitivity of 0.8214 and specificity of 0.2596.
...
```{r}
#creating a boxplot displaying distribution of peak vertical force asymetry in those who experienced lower body injury vs those that did not
...
```{r}
#creating a boxplot displaying distribution of maximum jump height in those who experienced lower body injury vs those that did not
...
This code with different variables and/or sports switched out was used multiple times. 

```{r}
#creates a density plot for jump height across sports
...
```{r}
#performs a Welch's T-test for difference of means of different variables between the court and field sports
...
```{r}
#uses a welch two sample t-test to test for differences between sports: filters for just two to see differences between all sports
...
```{r}
#creating a dataset containing all information for HJ
hj <- rbind(HJ_B, HJ_S, HJ_L, HJ_V) %>%
  mutate(Sport.2 = ifelse(Sport == 'Basketball' | Sport == 'Volleyball', 'Court', 'Field'))
```

```{r}
#creates a density plot for mean contact time across sports
...
```{r}
#uses a t-test to test for differences in means between only two sports: filters for these two sports
...
```{r}
#performs a Welch's T-test for difference of means between variables of interest and sport type
...
```{r}
#uses t-test for just two sports within the larger dataset
...
```{r}
#creates a density plot of hip abduction/adduction impulse across the 4 sports
...
```{r}
#uses welch two sample t-test for difference of means between two sports filtered for
